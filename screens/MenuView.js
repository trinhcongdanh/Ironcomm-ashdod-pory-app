...
import {
    ...
    menu_contact_index,
    menu_status_info_index, menu_log_out_index,
} from '../resource/BaseValue';
import {
    areYouAreLogout, cancel,
    menu_about,
    menu_another_item,
    menu_contact, menu_icon,
    menu_issue_page, menu_logout, menu_new_issue,
    menu_other_item, ok_text, statusInformation, version,
...
export default function MenuView({ onItemSelected }) {
    ...
    return (
        <View style={{flexDirection:"column", alignItems:"flex-start",
            height:screenHeight,width:screenWidth, backgroundColor:c_background_issue_item}}>
            <TouchableOpacity
                onPress={()=>onItemSelected(0)}>
                <Image
                    source={require("../image/icon_menu_blue.png")}
                    resizeMode="contain"
                    style={{width:screenWidth*0.05,height:screenWidth*0.05, margin:10}}
                />
            </TouchableOpacity>
            <View style={{width:screenWidth, height:1, backgroundColor:c_bg_issue_description}}></View>
            <TouchableOpacity onPress={()=>onItemSelected(menu_issue_page_index)} style={{flexDirection:'row'}}>
