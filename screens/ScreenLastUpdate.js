...
import {
    View, Text, Dimensions, Image, StyleSheet, FlatList, TouchableOpacity, TouchableWithoutFeedback, ActivityIndicator,
} from 'react-native';
import {
    lastUpdateTitle,
    sortByDateCreated, sortByPrior, sortByStatus,
    sortScreenTitle, updated,
...
export default class LastUpdateScreen extends React.Component {
    ...
    render () {
        ...
        return (
            <View style={{width:screenWidth, flex:1, flexDirection:'column',backgroundColor:c_background_issue_item}}>
                <View style={{flexDirection:'row', alignItems:'center',padding:15}}>
                    ...
                    <Text style={[mStyle.textTitle, {textAlign:'center',color:"#000000"}]}>
                        {lastUpdateTitle}
                    </Text>
                    <View style={{flex:1}}></View>
                </View>
                <View style={{width:screenWidth, height:1, backgroundColor:c_bg_issue_description}}></View>
                <FlatList
                    style={{ marginTop:10}}
                    ...
                    renderItem={({item, index}) =>
                        <TouchableOpacity
                            ...
                            style={{flexDirection:"row",marginBottom:10,paddingTop:10, paddingBottom:10, backgroundColor: c_background_issue_item,
                                alignItems:'flex-start', borderBottomColor:c_bg_line, borderBottomWidth:1}}>
                            <Image
                                source={require("../image/icon_notification_blue.png")}
                                resizeMode="cover"
                                style={{width:screenWidth*0.07, height:screenWidth*0.07*(48/43), marginStart:10}}
                            />
                            <View style={{flexDirection:'column', flex:1, alignItems: "flex-start", marginStart:10, marginEnd:5}}>
                                <Text style={[mStyle.textIssueName]}>{item.issue_name}</Text>
                                <Text style={[mStyle.textUpdateText]}>{item.update_text}</Text>
                                <Text style={[mStyle.textUpdateTime]}>{updated + " " + moment(item.created_on).format("DD/MM/YYYY")}</Text>
                            </View>
                            <Image
                                source={require("../image/icon_arrow_left_black.png")}
                                resizeMode="cover"
                                style={{width:screenWidth*0.03, height:screenWidth*0.03*(24/14), marginEnd:10}}
                            />
                        </TouchableOpacity>
                    }
